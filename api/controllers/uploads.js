...
const storage = multer.diskStorage({
  destination: async ({ body: { id, types }, url }, { originalname }, cb) => {
    ...
    if (supportingDocuments[types[0]] === supportingDocuments[0]) {
      try {
        if (fs.existsSync(`./uploads/${ostarbaiter?.id}/images`)) {
          const nameFile = (
            await readdir(`./uploads/${ostarbaiter?.id}/images`)
          )[0];
          if (nameFile) {
            fs.unlink(
              `./uploads/${ostarbaiter?.id}/images/${nameFile}`,
              () => {}
            );
          }
        }
    ...
    fs.mkdirSync(
      path.join(path.resolve("./uploads"), id, supportingDocuments[types[0]]),
      { recursive: true }
    );

    cb(null, `./uploads/${id}/${supportingDocuments[types[0]]}`);
  },
...
export default {
  ...
  async delete({ query: { file, type, id } }, res) {
    ...
    try {
      fs.unlink(
        `./uploads/${id}/${supportingDocuments[type]}/${file}`,
        () => {}
      );
    } catch (e) {
